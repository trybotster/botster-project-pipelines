...
3. Call `project_pipelines.spawn_ticket_session` for `botster_stack_plan` with an explicit
   ...
   bounded timeout. Capture the hold command pid and its `botster-session-worker` ancestor
   with fail-closed process queries. Missing worktree, ready file, hold pid, or worker
   identity fails the gate. Typed daemon errors, `EPERM`, failed `ps`/`pgrep`, and unknown
   kill errors also fail the gate instead of counting as absence.
